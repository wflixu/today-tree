...
export class FileEntry {
  ...
  public static checkRawFile(file: IFileEntryItem) {
    if (file === null || typeof file !== "object") {
      throw new TypeError(
        'Item must be IFileEntryItem object. See docs for more info',
      );
    }
    if (file.type !== FileType.Directory && file.type !== FileType.File) {
      throw new TypeError(
        `IFileEntryItem must have a 'type' property which is either FileType.File or FileType.Directory`,
      );
    }
    if (typeof file.name !== "string") {
      throw new TypeError(
        `IFileEntryItem must have a 'name' property of type string`,
      );
    }
  }
  ...
  private resolvedPathCache: string | null = null;

  protected constructor(
    root: Root | null,
    tree: ITreeSupervisor,
    ...
    this._fileName = parent ? root.pathfx.basename(fileName ?? '') : fileName;
  ...
  get path(): string {
    if (!this.parent) {
      throw new Error(
        `orphaned/detached FileEntries don't have path (except Root)`,
      );
    }
    if (!this.resolvedPathCache && this.fileName) {
   ...
   */
  public mv(to: Directory | null, fname: string = this.fileName) {
    const prevParent = this._parent;
    if (to === null || to.type !== FileType.Directory) {
      // that's the best check we can do; `parent instanceof Directory` causes a cyclic dependency
      this.dispose();
      this._parent = null;
    ...
    if (
      didChangeParent ||
      fname !==
      this.fileName /* change in filename means change in sort order */
    ) {
      this._fileName = fname;
      if (didChangeParent && prevParent) {
  ...
  public addMetadata(withKey: string, value: any) {
    if (!(withKey in this._metadata)) {
      ...
      this._superv.notifyDidChangeMetadata(this, {
      ...
      this._metadata[withKey] = value;
      this._superv.notifyDidChangeMetadata(this, {
        type: MetadataChangeType.Updated,
        key: withKey,
        prevValue,
        value,
      });
