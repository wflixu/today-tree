...
import { FileType, type IWatcherEvent, WatchEvent, type WatchTerminator } from './types'
import type { ITreeSupervisor } from './types'
...
export class Directory extends FileEntry {
	...
	protected flattenedBranch: Uint32Array | null = null
	...
	public mv(to: Directory | null, newName: string = this.fileName) {
	...
	protected expandBranch(branch: Directory) {
		...
		if (this !== branch && this.flattenedBranch && branch.flattenedBranch) {
	...
	protected async hardReloadChildren() {
		...
		this.hardReloadPromise.then(() => {
			this.hardReloadPromise = undefined
			this.hardReloadPResolver = undefined
		})
